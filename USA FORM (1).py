...
def authenticate(username, password):
    ...
    try:
        ...
        cursor.execute("SELECT role, product FROM users WHERE LOWER(username) = LOWER(?) AND password = ?", 
...
""")
        ...
        cursor.execute("""
            CREATE TABLE IF NOT EXISTS system_settings (
                ...
                killswitch_enabled INTEGER NOT NULL DEFAULT 0,
                chat_killswitch_enabled INTEGER NOT NULL DEFAULT 0)
        """)
        ...
        agents = [
            ("Karabila Younes", "30866", "agent", "LM_CS_LMUSA_EN"),
            ("Kaoutar Mzara", "30514", "agent", "LM_CS_LMUSA_ES"),
            ("Ben Tahar Chahid", "30864", "agent", "LM_CS_LMUSA_EN"),
            # Add all other agents...
        ]
        
        for username, password, role, product in agents:
            try:
                cursor.execute("""
...
def send_group_message(sender, message):
    if is_killswitch_enabled() or is_chat_killswitch_enabled():
        st.error("Chat is currently locked")
        return False
...
def get_user_bookings(username, booking_date):
    conn = get_db_connection()
    try:
        cursor = conn.cursor()
        cursor.execute("""
        ...
        with st.form("login_form"):
            ...
            if st.form_submit_button("Login"):
                if username and password:
                    role, product = authenticate(username, password)
                    if role:
    ...
    if st.session_state.role == "admin":
            ...
            if uploaded_file:
                try:
                    ...
                    if not all(col in df.columns for col in required_cols):
                        st.error("Missing required columns in the file")
                    else:
                        conn = get_db_connection()
                        try:
                            cursor = conn.cursor()
                            ...
                            conn.commit()
                            st.success("Break schedule uploaded successfully!")
                            st.rerun()
                        except sqlite3.Error as e:
        ...
        for product, product_breaks in lunch_by_product.items():
            ...
            for break_slot in product_breaks:
                b_id, name, start, end, max_u, product, created_by, ts = break_slot
        ...
        # Store current edits
        for b in breaks:
            b_id, name, start, end, max_u, product, created_by, ts = b
...
def show_quality_issues_section():
...
def show_midshift_issues_section():
